...
type GithubAccessTokenResponse = z.infer<typeof GithubAccessTokenResponseSchema>;
...
async function fetchUserDataFromGithub(accessToken: string): Promise<GithubUserResponse> {
...
function normalizeGithubUser(githubUser: GithubUserResponse): z.infer<typeof UserSchema> {
  if (!githubUser.email) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "GitHub email is required. Please make your email public on GitHub.",
    });
  }
